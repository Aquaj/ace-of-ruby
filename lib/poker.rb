...
def betting_round(starting_player, community, show_community = true)
  loop do # TURN
  nobody_bet = true
  (starting_player+1..starting_player+NUM_OF_PLAYERS).each do |player_index|
    player = PLAYERS[player_index%NUM_OF_PLAYERS]
...
def lets_bet(player)
  ...
  they_chose = false
  until they_chose
    answer = get_input("Do you wanna #{call_or_check current} the bet, (r)aise or (f)old ?")
    case answer
    when "f"
      player.fold
      they_chose = true
...
NUM_OF_PLAYERS = get_input("How many players are involed?").to_i
fail ArgumentError, "We need some players !" if NUM_OF_PLAYERS <= 0
puts

d = Deck.new()
PLAYERS = NUM_OF_PLAYERS.times.map { |e| Player.new(INIT_STAKE, d, "Player #{e+1}") }
...
(big_blinder+1..big_blinder+NUM_OF_PLAYERS).each do |player_index|
  player = PLAYERS[player_index%NUM_OF_PLAYERS]
  player.pick(2)
end

betting_round(big_blinder, show_community=false) # PRE-FLOP
